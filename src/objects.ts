...
export function isValid(question: Question, answer: string): boolean {
    let val = false;
    question.type === "short_answer_question" ? (val = true)
    : question.options.includes(answer) ? (val = true)
    : (val = false);
    return val;
}
...
export function toShortForm(question: Question): string {
    let val = "" + question.id + ": " + question.name.substring(0, 10);
    return val;
}
...
export function publishQuestion(question: Question): Question {
    let newQuest = { ...question };
    question.published ?
        (newQuest.published = !question.published)
    :   (newQuest.published = true);
    return newQuest;
}
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    let newQuest = { ...oldQuestion };
    newQuest.published = false;
    newQuest.name = "Copy of " + oldQuestion.name;
    newQuest.id = id;
    return newQuest;
}
...
export function addOption(question: Question, newOption: string): Question {
    let newQuest = { ...question };
    newQuest.options = [...question.options, newOption];
    return newQuest;
}
...
export function mergeQuestion(
    ...
    let newQuest = { ...contentQuestion };
    newQuest.id = id;
    newQuest.name = name;
    newQuest.published = false;
    newQuest.points = points;
    return newQuest;
}
